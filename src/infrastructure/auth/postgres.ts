...
export async function seed(db: Sequelize) {
    const defaultUser = await User.findOne({ limit: 1, where: { username: 'test' } });
    if(defaultUser === null ){
        const salt = await genSalt()
        const password = await hashPassword('test', salt)
        await User.create({userId: uuid(), username: 'test', email: 'test@test.pl', password:password, salt: salt, createdAt: new Date(new Date().toUTCString()), updatedAt: new Date(new Date().toUTCString())})
    }
}